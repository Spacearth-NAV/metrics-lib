...
class MetricServer(ABC):
    ...
    @classmethod
    def create_server(cls, server_type: str, namespace: str, fixed_labels: dict[str, str], **kwargs) -> "MetricServer":
        """
        ...
        :param kwargs: Additional keyword arguments forwarded to the concrete server implementation.
        ...
        if server_type == "aws":
            ...
            server = AmazonCloudwatchMetricServer(namespace, fixed_labels, **kwargs)

        elif server_type == "prometheus":
            from .prometheus import PrometheusMetricServer

            logging.info("creating a Prometheus metric server")
            server = PrometheusMetricServer(namespace, fixed_labels, **kwargs)
